...
const Profile = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      logger.info(
        `🔄 [PROFILE] Sending update request for ${email}${password ? " (with password change)" : ""}`,
      );
      ...
      if (email !== user.email || password) {
        logger.info(
          `🔄 [PROFILE] Sensitive info changed, forcing re-login for ${email}`,
        );
        localStorage.removeItem("userInfo");
        setMessage({
          text: "Profile updated! Please log in again with your new credentials. 🔐",
          type: "success",
        });
        ...
        >
          ...
          <div className="md:col-span-1">
            <div className="floating-card bg-white p-8 text-center flex flex-col items-center">
              ...
              {user.role === "regular" && (
                <div
                  className={`mt-6 w-full py-2 px-4 rounded-xl text-sm font-bold flex items-center justify-center gap-2 ${user.paymentStatus === "paid" ? "bg-green-100 text-green-700" : user.isPlanReserved ? "bg-amber-100 text-amber-700" : "bg-orange-100 text-orange-700"}`}
                >
                  <CheckCircle size={18} />
                  {user.paymentStatus === "paid"
                    ? "Premium Active"
                    : user.isPlanReserved
                      ? `${user.selectedPlan.toUpperCase()} Reserved`
                      : "Free Tier"}
                </div>
              )}
            </div>
          ...
          <div className="md:col-span-2">
            <div className="floating-card bg-white p-8">
              <h3 className="text-2xl font-bold text-primary mb-6 border-b border-gray-100 pb-4">
                  ...
                  <div className="flex items-start gap-4 relative z-10">
                    ...
                    <div>
                      ...
                      <p className="text-amber-800/80 text-sm mb-4">
                        You have reserved the{" "}
                        <span className="font-bold">{user.selectedPlan}</span>{" "}
                        package, but the payment has not been completed yet.
                      </p>
                        ...
                        <button
                          onClick={() =>
                            (window.location.href =
                              "/checkout?plan=" + user.selectedPlan)
                          }
              ...
              {message.text && (
                <div
                  className={`p-4 mb-6 rounded-xl font-medium ${message.type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}
                >
                  {message.text}
                </div>
              )}
